...
class ProsodyPanel:
    ...
    def __init__(self, page: ft.Page) -> None:
        ...
        self._rate_slider  = ft.Slider(
            value=0, min=-100, max=100, divisions=200,
            on_change=self._on_rate_change,  expand=True,
        )
        self._vol_slider   = ft.Slider(
            value=0, min=-100, max=100, divisions=200,
            on_change=self._on_vol_change,   expand=True,
        )
        self._pitch_slider = ft.Slider(
            value=0, min=-200, max=200, divisions=400,
            on_change=self._on_pitch_change, expand=True,
        )
